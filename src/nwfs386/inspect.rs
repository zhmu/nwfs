...
use crate::nwfs386::{parser, partition, types};
...
pub fn inspect(f: &mut File, lba_start: u64) -> Result<()> {
    ...
    let partition = partition::NWPartition::new(f, lba_start * types::SECTOR_SIZE)?;
    ...
    dump_fat_chain(f, first_data_block_offset, volume_info.rootdir_block_nr)?;

    let mut current_entry = volume_info.rootdir_block_nr;
    while current_entry != 0xffffffff {
        let entry = get_fat_entry(f, first_data_block_offset, current_entry)?;
        ...
        read_dir_block(f, volume_block_size)?;
        current_entry = entry.1;
    }
    Ok(())
}
